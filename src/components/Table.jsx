						...
						return (
							<tr className="bg-violet-100 border-b hover:bg-violet-400" key={`columns-${index}`} >
								<td className="p-4 w-4">
									<div className="flex items-center">
										<input id={index} onChange={handleClick} checked={isCheck.includes(index)} type="checkbox" className="w-4 h-4 text-blue-600 bg-violet-100 rounded border-violet-300 focus:ring-blue-500 focus:ring-2 " />
									</div>
								</td>
								...
								</td>
								{
									row.results.map(result => {
										return (
											<td scope="row" className="py-4 px-6 font-medium text-violet-900 whitespace-nowrap">
												<input type="text" className="w-10 border border-violet-300 rounded-md shadow-sm focus:border-violet-300 focus:ring focus:ring-violet-200 focus:ring-opacity-50" value={result} />
											</td>
										)
									})
								}
