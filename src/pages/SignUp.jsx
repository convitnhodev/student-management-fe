...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let temp1 = !username || handlePass() || handleRepass() || !fullname;
    let temp2 = !address || handleEmail() || handlePhone() || !birthday;
    if (temp1 || temp2) {
      setErr({ ...err, total: "Vui lòng điền đầy đủ thông tin" });
      return;
    } else {
      const userData = {
        username,
      ...
      try {
        ...
        if (error.response.data.key === "User Existed") {
          setErr({ ...err, username: "Tài khoản đã tồn tại" });
        }
      }
    }
  };
    ...
    <section className="h-screen z-10 bg-violet-100 bg-fixed">
      <div className="container py-3 h-full">
        <div className="flex justify-center items-center flex-wrap g-6 text-gray-800">
          <div className="bg-violet-50 md:w-8/12 lg:w-4/12 lg:ml-20 border border-solid px-16 py-6 rounded">
            <h1 className=" mb-5 text-violet-800 font-bold text-4xl text-center">
              ĐĂNG KÝ
            </h1>
            <form onSubmit={handleSubmit}>
              ...
              {err.username && <p className="text-red-500">{err.username}</p>}
              <div className="mb-2">
                <input
                  type="password"
                  className="form-control block w-full px-4 py-2 text-xl font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
              ...
              <div className="mb-2">
                <input
                  ...
                  value={birthday}
                />
                {err.total && <p className="text-red-500">{err.total}</p>}
              </div>
              ...
              <div className="flex items-center my-1 before:flex-1 before:border-t before:border-gray-300 before:mt-0.5 after:flex-1 after:border-t after:border-gray-300 after:mt-0.5">
