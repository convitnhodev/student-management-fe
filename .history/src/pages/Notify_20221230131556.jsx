...
export default function Notify() {
  ...
  function handleCheck(e) {
    if (!checked.includes(e.target.name)) {
      ...
      let temp = checked.filter((item) => item !== e.target.name);
      setChecked(temp);
    } else {
      checked.push(id);
      setChecked(checked);
          ...
          <thead className="text-base text-violet-700 uppercase bg-violet-300">
            <tr>
              {data.column_names.map((name) => {
                return (
                  <th scope="col" key={name} className="py-3 px-20">
                    {name}
                  </th>
                );
              })}
            ...
            {data.rows.map((row, index) => {
              return (
                ...
                >
                  <td
                    scope="row"
                    className="py-4 px-20 font-medium text-violet-900 whitespace-nowrap"
                  >
                    {index + 1}
                  </td>
                  <td
